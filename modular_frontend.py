...
with st.sidebar:
    ...
    # Ensure topic_input is in session state
    ...
    if st.button("🎲 Run Daily Explorer"):
    ...
    if "explorer_topics" in st.session_state:
        ...
        if st.button("✨ Auto-Generate Selected"):
            # Set the topic in the widget state
            selected_topic = topics[selected_idx]
            st.session_state["topic_input"] = selected_topic.title
    ...
    if not past_files:
        ...
        if st.button("📂 Load selected blog"):
            if selected_md_file:
                ...
                # Update widget state
                st.session_state["topic_input"] = extract_title_from_md(md_text, selected_md_file.stem)
